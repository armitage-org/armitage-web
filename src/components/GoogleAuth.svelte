<script>
// This is a modified copy of https://github.com/beyonk-adventures/svelte-social-auth/blob/master/src/google-auth/GoogleAuth.svelte
...
<Button disabled="{disabled}" bind:element="{signinCta}" type="google">
  <span class="inline-flex items-center font-bold">
    <img
      class="w-7 h-7 mr-2 p-1 rounded-sm border-grey border hidden md:inline-block"
      ...
      alt="Google" />
    <slot>
      <span>Sign in<span class="hidden sm:block"> with Google</span></span>
    </slot>
  </span>
</Button>
